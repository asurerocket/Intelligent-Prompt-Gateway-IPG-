...
export class McpReport {
  public async exportAll(workspacePath: string | undefined, result: SecurityAssessmentResult): Promise<{ jsonPath: string; csvPath: string; htmlPath: string }> {
    ...
    const jsonPath = path.join(baseDir, `mcp-scan-${stamp}.json`);
    const csvPath = path.join(baseDir, `mcp-scan-${stamp}.csv`);
    const htmlPath = path.join(baseDir, `mcp-scan-${stamp}.html`);
  ...
  private toHtml(result: SecurityAssessmentResult): string {
    ...
    return `<!doctype html><html><head><meta charset="utf-8" /><title>MCP Security Report</title><style>body{font-family:Segoe UI,sans-serif;padding:16px;}table{border-collapse:collapse;width:100%;font-size:12px;}td,th{border:1px solid #ccc;padding:6px;}th{background:#f5f5f5;}</style></head><body><h2>MCP Security Assessment</h2><p>Score: ${result.score}/100 (${result.band})</p><table><thead><tr><th>Category</th><th>Check</th><th>Severity</th><th>Result</th><th>Score</th><th>Details</th></tr></thead><tbody>${rows}</tbody></table></body></html>`;
  }
}
